...
def make_keyboard(texts: Sequence[Sequence[str]], callbacks: Sequence[Sequence[str]]) -> InlineKeyboardMarkup:
    """Make a row with buttons"""
    buttons = [[InlineKeyboardButton(text=texts[x][y], callback_data=callbacks[x][y]) for x in range(len(texts[y]))] for y in range(len(texts))]
    result = InlineKeyboardMarkup(inline_keyboard=buttons)
    return result


def make_keyboard_things(texts: Sequence[Sequence[str]], acts: Sequence[Sequence[INPUT_FIELDS]]) -> InlineKeyboardMarkup:
...
TO_MENU_BUTTON = make_button("Меню", "menu")
MENU_BUTTONS = make_keyboard([["Ваши аккануты", "Изменить скины"], ["Политика конфиденциальности"]], [["account", "skin"], ["privacy"]])
ADMIN_MENU_BUTTONS = make_keyboard([["Ваши аккаунты", "Изменить скины"], ["Политика конфиденциальности", "Все аккаунты"]], [["account", "skin"], ["privacy", "admin"]])
